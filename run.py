...
def main():
    ...
    pygame.display.set_caption("Algorithm: IDA* Search")
    ...
    while running:
            ...
            if not simulation_started and event.type == pygame.MOUSEBUTTONDOWN:
                if button_rect.collidepoint(event.pos):
                    ...
                    if environment.task_locations:
                        nearest_task = agent.find_nearest_task()
                        if nearest_task:
                            agent.find_path_to_task(nearest_task)
        ...
        if agent.task_costs:
            ...
            for i, (task_num, cost) in enumerate(agent.task_costs.items()):
                if i > 0:
                    completed_tasks_text += ", "
                completed_tasks_text += f"{task_num}(Cost: {cost})"
            
            completed_tasks_surface = font.render(completed_tasks_text, True, TEXT_COLOR)
            screen.blit(completed_tasks_surface, (status_x, 110))
        
       
        total_cost_text = f"Total Path Cost: {agent.path_cost}"
